...
@section('content')
    <div class="card mb-4">
        <div class="card-header d-flex align-items-center justify-content-between">
            <h5 class="mb-0">Buat Surat Pengantar Rawat
                <span class="text text-primary">{{ $item->queue->patient->name ?? '' }}</span>
            </h5>
        </div>
        <div class="card-body">
            <form method="POST" action="{{ route('igd/suratpengantar.store', $item->id) }}">
                @csrf
                {{-- <div class="row mb-3">
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-sm-10">
                        <input type="text" name="diagnosa_primer" class="form-control" id="basic-default-name"
                            value="{{ $diagnosa[0] ?? '' }}" />
                    </div>
                </div>
                <div class="row mb-3">
                        ...
                        <div id="diagnosa_sekunder_container">
                            @if (empty($diagnosaSekunder))
                                <div class="input-group mb-2">
                                    <input type="text" name="diagnosa_sekunder[]" class="form-control me-1 rounded"
                            ...
                            @else
                                @foreach ($diagnosaSekunder as $sekunder)
                                    <div class="input-group mb-2">
                                        <input type="text" name="diagnosa_sekunder[]" class="form-control me-1 rounded"
                                            id="basic-default-name" value="{{ $sekunder ?? '' }}" />
                                        @if ($loop->first)
                                            <div class="input-group-addon">
                                                <button type="button" class="btn btn-dark" id="tambah"><i
                                                        class="bi bi-plus">Tambah</i></button>
                        ...
                        @if ($itemReqLab)
                            @foreach ($itemReqLab->laboratoriumRequestDetails as $reqLab)
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="persiapan_operasi[]"
                                        value="{{ $reqLab->laboratoriumRequestMasterVariable->name }}" checked>
                                    <label class="form-check-label"
                                        for="basic-default-name">{{ $reqLab->laboratoriumRequestMasterVariable->name ?? '' }}</label>
                                </div>
                            @endforeach
                        @endif
                        @if ($itemReqRadio)
                            @foreach ($itemReqRadio->radiologiFormRequestMasters as $reqRadio)
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="persiapan_operasi[]"
                                        value="{{ $reqRadio->name }}" checked>
                                    <label class="form-check-label"
                                        for="basic-default-name">{{ $reqRadio->name ?? '' }}</label>
                                </div>
                            @endforeach
                        @endif
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-sm-10">
                        <input type="datetime-local" name="tgl_operasi" class="form-control" id="tgl_operasi"
