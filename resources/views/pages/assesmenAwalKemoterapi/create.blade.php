...
@section('content')
    ...
    <form action="{{ route('kemoterapi/assesmenawal.store', $item->id) }}" method="POST">
        @csrf
        <div class="card">
            ...
            <div class="card-body">
                ...
                <p class="m-0 fw-bold mt-4">Status Performance Karnofsky</p>
                <table class="table">
                    <thead>
                        <tr class="text-nowrap">
                            <th class="text-body fw-bold">Deskripsi</th>
                            <th class="text-body fw-bold">Skor</th>
                            <th class="text-body fw-bold">Centang</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($arrDeskripsi as $index => $deskripsi)
                            <tr id="row{{ $index }}">
                                <td>
                                    {{ $deskripsi }}
                                    <input type="hidden" name="deskripsi[{{ $index }}][name]"
                                        value="{{ $deskripsi }}">
                                </td>
                                <td>
                                    {{ $arrSkor[$index] }}
                                    <input type="hidden" name="skor[{{ $index }}][score]"
                                        value="{{ $arrSkor[$index] }}">
                                </td>
                                ...
                                <td>
                                    <div class="form-check form-check-sm form-check-inline">
                                        <input class="form-check-input" type="checkbox"
                                            name="skor[{{ $index }}][isChecked]" id="skor[{{ $index }}]"
                                            value="{{ $arrSkor[$index] }}" />
                                    </div>
                                </td>
                ...
                <p id="totalSkor" class="m-3 fw-bold">Total Skor Karnofsky : 0</p>
                <p class="m-0 text-muted fw-light fst-italic">Catatan : Pasien dapat dilakukan kemoterapi bila skor
                    Karnofsky > 60</p>

                <p class="m-0 fw-bold mb-3 mt-4">Rencana</p>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-8 d-flex align-self-center">
                        <button type="button" class="btn btn-dark btn-sm"
                            onclick="tambahEdukasi([this, 'edukasi[]'])">+</button>
                    </div>
                </div>
            </div>
            ...
            <div class="row mb-4">
                ...
                <div class="col-4 text-center">
                    <input type="text" class="form-control form-control-sm text-center"
                        value="{{ auth()->user()->name ?? '' }}" id="petugas_name" disabled>
                </div>
            </div>
    ...
    {{-- total skor karnofsky --}}
    <script>
        // Fungsi untuk menghitung total skor dan memperbarui tampilan
        function updateTotalSkor() {
            let totalSkor = 0;
            document.querySelectorAll('input[type="checkbox"]:checked').forEach(function(checkbox) {
                totalSkor += parseInt(checkbox.value);
            });
            document.getElementById('totalSkor').innerText = 'Total Skor Karnofsky : ' + totalSkor;
        }

        // Event listener untuk memanggil fungsi updateTotalSkor saat checkbox diubah
        ...
        // Panggil fungsi pertama kali saat halaman dimuat untuk menginisialisasi total skor
        updateTotalSkor();
    </script>
@endsection
