    ...
    <div class="card p-3 mt-5 ">
        ...
        <div class="d-flex">
            <h4 class="align-self-center m-0">PENERAPAN BUNDLES PENCEGAHAN INFEKSI RUMAH SAKIT (HAIs)
                INFEKSI SALURAN KEMIH -- {{ $data->rawatInapPatient->queue->patient->name }}</h4>
            {{-- <div class="ms-auto">
        ...
        <form action="{{ route('hais/isk.update', $data->id) }}" method="POST"
            ...
            <div class="card-body table-responsive">
                ...
                <table class="w-100 mt-5">
                    ...
                    @foreach ($details as $detail)
                        @if ($detail->kategori == 'Insersi')
                            <tr class="text-center">
                                ...
                                <input type="hidden" name="kategori[{{ $detail->id }}]" value="Insersi">
                                @if ($detail->status == 'Y')
                    ...
                    @foreach ($details as $detail)
                        @if ($detail->kategori == 'Maintenance')
                            <tr class="text-center">
                                <td class="pt-3 text-start">{{ $detail->nama }}</td>
                                <input type="hidden" name="id_akhir" value="{{ $detail->id }}">
                                <input type="hidden" name="id[{{ $detail->id }}]" value="{{ $detail->id }}">
                                <input type="hidden" name="nama[{{ $detail->id }}]" value="{{ $detail->nama }}">
                                <input type="hidden" name="kategori[{{ $detail->id }}]" value="Maintenance">
                                @if ($detail->status == 'Y')
                                    <td class="p-0 pt-3"> <input class="form-check-input"
                                            name="status[{{ $detail->id }}]" type="radio" value="Y" checked />
                                    </td>
                    ...
                    <tr>
                        ...
                        <td colspan="4"><input type="text" value="{{ $data->user->name }}"
