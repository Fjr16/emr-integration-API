...
@section('content')
    ...
    <div class="card p-3 mt-5 ">
        <div class="card-header">
            <div class="d-flex">
                <h4 class="align-self-center m-0">Edit RENCANA PULANG (<em>DISCHARGE PLANNING</em>)</h4>
            </div>
            <hr class="m-0 mt-2 mb-3">
        </div>
        <div class="card-body">
            <form action="{{ route('checklist/rencana/pulang/page/one.update', $item->id) }}" method="POST"
                    ...
                    <tr>
                                ...
                                @foreach ($arrAktivitas as $aktivitas)
                                    <div class="col-12 form-check form-check-inline my-4 ">
                                        <input class="form-check-input" type="checkbox" id="{{ $aktivitas }}"
                                            ...
                                            {{ in_array($aktivitas, $arrDetail[0]) ? 'checked' : '' }} />
                        ...
                        <td>
                            <div class="row">
                                @foreach ($arrAktivitas as $aktivitas)
                                    @php
                                        $AktivitasCatatan = App\Models\RanapDetailDischargePlanningPerawatPatient::where(
                                            'ranap_discharge_planning_perawat_patient_id',
                                            $item->id,
                                        )
                                            ->where('kegiatan', 'AKTIFITAS, EDUKASI, DAN LATIHAN')
                                            ->first();
                                        $catatan = App\Models\RanapChildDetailDischargePlanningSurgery::where(
                                            'ranap_detail_discharge_planning_perawat_patient_id',
                                            $AktivitasCatatan->id,
                                        )
                                            ->where('name', $aktivitas)
                                            ->first();
                                    @endphp
                                    <div class="col-12 mt-4">
                                        <textarea class="form-control form-control-sm" rows="2" placeholder="Catatan {{ $aktivitas }}"
                                            name="catatan_aktifitas[]"> {{ $catatan->value ?? '' }}</textarea>
                                    </div>
                                @endforeach
                            </div>
                        </td>
                                ...
                                @foreach ($arrFasilitas as $fasilitas)
                                    <div class="col-12 form-check form-check-inline mt-3 ">
                                        <input class="form-check-input" type="checkbox" id="{{ $fasilitas }}"
                                            ...
                                            {{ in_array($fasilitas, $arrDetail[1]) ? 'checked' : '' }} />
                                        <label class="form-check-label"
                                ...
                                @foreach ($arrFasilitas as $fasilitas)
                                    @php
                                        $AktivitasCatatan = App\Models\RanapDetailDischargePlanningPerawatPatient::where(
                                            ...
                                            ->where(
                                                'kegiatan',
                                                'FASILITAS KESEHATAN YANG BISA DIHUBUNGI JIKA TERJADI KEDARURATAN',
                                            )
                                            ->first();
                                        $catatan = App\Models\RanapChildDetailDischargePlanningSurgery::where(
                                            'ranap_detail_discharge_planning_perawat_patient_id',
                                    ...
                                    <textarea class="form-control form-control-sm my-3" rows="2" placeholder="Catatan Untuk {{ $fasilitas }}"
                                        name="catatan_fasilitas[]">{{ $catatan->value }}</textarea>
                                @endforeach
                            </div>
                        </td>
                    </tr>
                                ...
                                <div class="col-12 form-check form-check-inline mt-3 ">
                                    ...
                                    @else
                                        <input class="form-check-input" type="checkbox" id="{{ $rincian }}"
                                            value="{{ $rincian }}" name="rincian[]"
                                            {{ in_array($rincian, $arrDetail[2]) ? 'checked' : '' }} />
                                        <label class="form-check-label"
                                                    ...
                                                    @foreach ($arrPendamping as $pendamping)
                                                        <div class="col-2">
                                                            <input class="form-check-input" type="radio"
                                                                ...
                                                                name="pendamping"
                                                                {{ $grandChildPendamping == $pendamping ? 'checked' : '' }} />
                                                            <label class="form-check-label"
                                                    ...
                                                    @foreach ($arrTrans as $transportasi)
                                                        <div class="col-3">
                                                            <input class="form-check-input" type="radio"
                                                                ...
                                                                name="transportasi"
                                                                {{ $grandChildTransportasi == $transportasi ? 'checked' : '' }} />
                                                            <label class="form-check-label"
                                                    ...
                                                    @foreach ($arrKelAdm as $adm)
                                                        <div class="form-check form-check-inline mt-3 col-3">
                                                            <input class="form-check-input" type="checkbox"
                                                                id="{{ $adm }}" value="{{ $adm }}"
                                                                name="adm[]"
                                                                {{ in_array($adm, $grandChildKelAdmArr) ? 'checked' : '' }} />
                                                            <label class="form-check-label"
                                                                for="{{ $adm }}">{{ $adm }}</label>
                                                        </div>
                                                    @endforeach
                ...
                <div class="row mb-5">
                    <div class="col-4 text-center">
                        <img src="{{ asset('storage/' . $item->ttd_pasien) }}" alt="" id="ImgTtdKeluargaPasien">
                        <textarea id="ttd" name="ttd_pasien" style="display: none;">{{ $item->ttd_pasien ?? '' }}</textarea>
                    ...
                    <div class="col-4"></div>
                    <div class="col-4 text-center">
                        <img src="{{ asset('storage/' . $item->ttd_petugas) }}" alt="" id="ImgTtdPpj">
                        <textarea id="ttdPpj" name="ttd_petugas" style="display: none;">{{ $item->ttd_petugas ?? '' }}</textarea>
                ...
                <div class="row">
                    <div class="col-4 text-center">
                        <input type="text" class="form-control" name="pasien_name" id="pasien_name"
                            placeholder="Nama Pasien / Keluarga Pasien" value="{{ $item->pasien_name ?? '' }}">
                    </div>
                    <div class="col-4"></div>
                    <div class="col-4 text-center d-flex">
                        <input type="text" class="form-control" name="petugas_name" id="petugas_name"
                            placeholder="Nama Perawat Penanggung Jawap Pasien" value="{{ $item->petugas_name ?? '' }}"
                            @readonly(true)>
                    </div>
                </div>
