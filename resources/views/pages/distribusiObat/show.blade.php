...
<div class="card p-3 mt-5">
  ...
  <div class="d-flex">
    <h4 class="align-self-center m-0">DATA DISTRIBUSI OBAT KE UNIT {{ $category->unit->name }} - {{ $category->unitCategoryPivot->name }}</h4>
    <a href="{{ route('farmasi/obat/distribusi.create', $category->id) }}" class="btn btn-dark btn-sm m-0 mx-3">+</a>
  </div>
          ...
          <td>
            <div class="dropdown">
              ...
              <div class="dropdown-menu">
                  <form action="{{ route('farmasi/obat/distribusi.destroy', $item->medicineDistribution->id) }}" method="POST">
                      @csrf
                      @method('DELETE')
