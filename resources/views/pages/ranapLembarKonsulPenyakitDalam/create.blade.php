...
@section('content')
  ...
  <div class="card p-3 mt-5">
    ...
    <div class="card-body">
      <form action="{{ route('lembar/konsultasi/penyakit/dalam.store', $item->id) }}" method="POST">
        @csrf
        <!--  -->
        <div class="row">
        ...
        <div class="mx-5">
          ...
          <div class="form-check">
            <input type="radio" class="form-check-input" name="permintaan" value="{{ $permintaan }}" id="permintaan_{{ $loop->iteration }}" {{ $loop->first ? 'checked' : '' }}>
            <label for="permintaan_{{ $loop->iteration }}" class="form-check-label">{{ $loop->iteration }}. {{ $permintaan }}</label>
          </div>
          @endforeach
        </div>
        <div>
          ...
          <div class="form-floating">
            <textarea class="form-control" id="editor" name="ket_pasien" style="height: 50px"></textarea>
          </div>
        </div>
        <div>
          <p class="fw-semibold mx-2  mt-5">Pemeriksaan yang ditemukan : </p>
          <div class="form-floating">
            <textarea class="form-control" id="editor2" name="pemeriksaan_ditemukan" style="height: 125px"></textarea>
          </div>
        </div>
