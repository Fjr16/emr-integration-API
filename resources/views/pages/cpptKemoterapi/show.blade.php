  ...
  <div class="page">
    ...
    <div class="content">
      <table class="table-bordered w-100 mt-4">
        ...
        <tbody>
          @foreach ($cppt_kemoterapi as $cppt)
          @php
            $soap = str_replace(['|', ',', '&nbsp;'], '', $cppt->soap);
          @endphp
          @if ($cppt->changeLogKemoterapi->isNotEmpty())
            @foreach ($cppt->changeLogKemoterapi as $change)
              @php
              ...
              <tr class="text-danger" style="text-decoration: line-through;">
                ...
                <td class="text-center">{{ $change->user->name ?? '' }} ({{ $change->user->staff_id }})</td>
          ...
          <tr>
                    ...
                    <tr>
                      <td>
                        ...
                        @else
                          @if (auth()->user()->id == $cppt->user->id)
                            <span><button class="btn btn-sm btn-dark" id="ttd_user" type="button" onclick="openModal({{ $cppt->id }}, 'ttd_user')">Tanda Tangan</button></span><br>
                          @else
                      ...
                      <td>
                        @if ($cppt->ttd_dpjp)
                          ...
                          @endphp
                          {{-- <span>Divalidasi oleh DPJP  <span class="fw-bold">({{ $cppt->kemoterapiPatient->user->name }})</span> pada <span class="fw-bold">{{ $tanggalValidasiDpjp->format('d M Y') }}</span> --}}
                          <span>Divalidasi oleh DPJP  <span class="fw-bold">({{ $cppt->kemoterapiPatient->kemoterapiDpjpPatientDetails->where('user_id', $cppt->id_dpjp)->first()->user->name ?? '' }})</span> pada <span class="fw-bold">{{ $tanggalValidasiDpjp->format('d M Y') }}</span>
                            <img src="{{ Storage::url($cppt->ttd_dpjp) }}" alt="" class="img-thumbnail">
                        @else
                          @if (auth()->user()->id  == $cppt->kemoterapiPatient->kemoterapiDpjpPatientDetails->where('status', true)->first()->user_id)
                            <span><button class="btn btn-sm btn-dark" type="button" onclick="openModal({{ $cppt->id }}, 'ttd_dpjp')">Validasi</button></span>
                          @else
                ...
                <table class="table">
                  ...
                  <tbody>
                    <tr>
                      ...
                      <td>
                        @if ($cppt->kemoterapiCpptAlihRawatPatient)     
                          @if ($cppt->kemoterapiCpptAlihRawatPatient->ttd_user)
                            @php
                              $tanggalValidasiDpjpTujuan = Carbon\Carbon::parse($cppt->kemoterapiCpptAlihRawatPatient->tanggal);
                            @endphp
                            <span>Dikonfirmasi oleh <span class="fw-bold">({{ $cppt->kemoterapiCpptAlihRawatPatient->user->name }})</span> pada <span class="fw-bold">{{ $tanggalValidasiDpjp->format('d M Y') }}</span>
                              <img src="{{ Storage::url($cppt->kemoterapiCpptAlihRawatPatient->ttd_user) }}" alt="" class="img-thumbnail">
                          @else
                            @if (auth()->user()->id  == $cppt->kemoterapiCpptAlihRawatPatient->user_id)
                              <span><button class="btn btn-sm btn-dark" type="button" onclick="openModal({{ $cppt->id }}, 'ttd_user_alih_rawat')">Konfirmasi</button></span>
                            @else
            ...
            <td class="text-center">
              @if ($cppt->kemoterapiCpptSbarPatient)
                <img src="{{ Storage::url($cppt->kemoterapiCpptSbarPatient->ttd) }}" alt="" class="img-thumbnail">
                <span class="text-center">
                  ({{ $cppt->kemoterapiCpptSbarPatient->user->name }}) <br>
                  @php
                    $tanggal = Carbon\Carbon::parse($cppt->kemoterapiCpptSbarPatient->tanggal);
                  @endphp
                  {{ $tanggal->format('d M Y') }} 
                </span>
              @else
              @if (auth()->user() && $cppt->kemoterapiPatient && $cppt->kemoterapiPatient->kemoterapiDpjpPatientDetails->where('status', true)->first() && auth()->user()->id == $cppt->kemoterapiPatient->kemoterapiDpjpPatientDetails->where('status', true)->first()->user_id)
              <span><button class="btn btn-sm btn-dark" type="button" onclick="openModal({{ $cppt->id }}, 'ttd')">Verifikasi</button></span>
                @else
