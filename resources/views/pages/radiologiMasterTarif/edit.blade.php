...
<div class="card p-3 mt-5">
  <div class="d-flex">
    ...
    <div class="ms-auto">
      <a href="{{ route('rajal/master/tarif/radiologi.create', $item->id) }}" class="btn btn-success btn-sm m-0">Sinkronisasi Ulang</a>
      <a href="{{ route('rajal/master/tarif/radiologi.index') }}" class="btn btn-success btn-sm m-0">Kembali</a>
    </div>
  </div>
    <hr class="m-0 mt-2 mb-3">
    <h6 class="">Tarif Tindakan Semua Status Pasien</h6>
    <div class="table-responsive text-nowrap">
      <table class="table">
        ...
        <tbody>
          <form action="{{ route('rajal/master/radiologi.update', $item->id) }}" method="POST">
            @method('PUT')
            @csrf
          <tr>
...
<div class="card p-3 mt-3">
    ...
    <div class="table-responsive text-nowrap">
      <table class="table">
        ...
        <tbody>
          @foreach ($item->radiologiFormRequestMasterRates as $rate)
          <form action="{{ route('rajal/master/tarif/radiologi.update', $rate->id) }}" method="POST">
            @method('PUT')
            @csrf
            <tr>
