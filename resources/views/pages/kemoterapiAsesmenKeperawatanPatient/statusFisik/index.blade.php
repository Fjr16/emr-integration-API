  ...
  <div class="card mb-4">
    <div class="card-header m-0">
        ...
        <div class="col-3 m-0 text-end">
            <a href="{{ route('kemoterapi/assesmen/awal/keperawatan.detail', $item->patient_id) }}" class="btn btn-success btn-sm">Kembali</a>
        </div>
        </div>
        <div class="row m-auto mt-2">
          <a href="{{ route('kemoterapi/asesmen/status/fisik.index', $item->id) }}" class="btn {{ Route::is('kemoterapi/asesmen/status/fisik.index*') ? 'btn-primary' : '' }} border btn-sm col-3">Status Fisik</a>
          {{-- <a href="{{ route('kemoterapi/asesmen/skrining/resiko/jatuh.index', $item->id) }}" class="btn {{ Route::is('kemoterapi/asesmen/skrining/resiko/jatuh.index*') ? 'btn-primary' : '' }} border btn-sm col-3">Skrining Resiko Jatuh</a>
          <a href="{{ route('kemoterapi/asesmen/diagnosis/keperawatan.index', $item->id) }}" class="btn {{ Route::is('kemoterapi/asesmen/diagnosis/keperawatan.index*') ? 'btn-primary' : '' }} border btn-sm col-3">Diagnosis Keperawatan</a>
          <a href="{{ route('kemoterapi/asesmen/rencana/asuhan.index', $item->id) }}" class="btn {{ Route::is('kemoterapi/asesmen/rencana/asuhan.index*') ? 'btn-primary' : '' }} border btn-sm col-3">Rencana Asuhan</a> --}}
        </div>
    </div>

    <div class="card-body">
      <h6 class="text-center bg-dark text-white py-2">STATUS FISIK</h6>
      <form action="{{ route('kemoterapi/asesmen/status/fisik.store', $item->id) }}" method="POST">
      @csrf
      <div class="row mb-3">
        <div class="col-sm-4 ">
        ...
        <div class="row">
          <div class="col-sm-4 ">
            ...
            @foreach ($asesmentNyeri as $nyeri)
              <div class="form-check">
                <input class="form-check-input" name="nyeri-hilang[]" type="checkbox" value="{{$nyeri}}" id="defaultCheck1" />
                <label class="form-check-label" for="defaultCheck1">
                  {{ $nyeri }}
                </label>
              </div>
