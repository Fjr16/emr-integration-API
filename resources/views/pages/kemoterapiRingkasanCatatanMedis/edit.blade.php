...
@section('content')
    ...
    <div class="card p-3 mt-5">
        <h5>
            UPDATE RINGKASAN CATATAN MEDIS PASIEN KELUAR KEMOTERAPI
            <span class="fst-italic">(DISCHARGE SUMMARY)</span>
        </h5>
        <form action="{{ route('ringkasan/catatan/medis/kemoterapi.update', $discharge->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="d-flex mb-3">
                <div class="col-4">
                    ...
                    <input type="datetime-local" class="form-control" name="tanggal_masuk"
                        value="{{ old('tanggal_masuk', $discharge->tanggal_masuk) }}">
                </div>
                <div class="col-4 ms-auto">
                    ...
                    <input type="datetime-local" class="form-control" name="tanggal_keluar"
                        value="{{ old('tanggal_keluar', $discharge->tanggal_keluar) }}">
                </div>
            </div>
            <div class="row mb-4">
                <div class="col">
                    ...
                    <input type="text" class="form-control" name="dokter_pengirim"
                        value="{{ $discharge->dokter_pengirim ?? '' }}" readonly>
                </div>
            </div>
            <div class="row mb-5">
                <h6 class="col">INDIKASI PASIEN DIRAWAT</h6>
                <div class="form-floating">
                    <textarea id="editor" class="form-control" name="indikasi" style="height: 100px">{{ old('indikasi', $discharge->indikasi) }}</textarea>
                </div>
            </div>
            <div class="row mb-5">
                <h6 class="col">ANAMNESIS</h6>
                <div class="form-floating">
                    <textarea id="editor2" class="form-control" name="anamnesis" style="height: 100px">{{ old('anamnesis', $discharge->anamnesis) }}</textarea>
                </div>
            </div>
            <div class="row mb-5">
                <h6 class="col">RIWAYAT PENYAKIT</h6>
                <div class="form-floating">
                    <textarea id="editor3" class="form-control" name="riwayat_penyakit" style="height: 100px">{{ old('riwayat_penyakit', $discharge->riwayat_penyakit) }}</textarea>
                </div>
            </div>
            <div class="row mb-5">
                <h6 class="col">PEMERIKSAAN FISIK</h6>
                <div class="form-floating">
                    <textarea id="editor4" class="form-control" name="pemeriksaan_fisik" style="height: 100px">{{ old('pemeriksaan_fisik', $discharge->pemeriksaan_fisik) }}</textarea>
                </div>
            </div>
            <div class="row mb-5">
                <h6 class="col">PEMERIKSAAN DIAGNOSTIK</h6>
                <div class="form-floating">
                    <textarea id="editor5" class="form-control" name="pemeriksaan_diagnostik" style="height: 100px">{{ old('pemeriksaan_diagnostik', $discharge->pemeriksaan_diagnostik) }}</textarea>
                </div>
            </div>
            <div class="row mb-5">
                <div class="col-10">
                    <h6 class="col">DIAGNOSA UTAMA</h6>
                    <textarea id="editor6" name="diagnosa_utama" class="form-control" style="height: 100px">{{ is_array($diagnosaUtamaLama) ? implode(', ', $diagnosaUtamaLama) : $diagnosaUtamaLama }}</textarea>

                </div>
                <div class="col-2">
                    <p>ICD 10</p>
                    <div class="row">
                        {{-- @foreach (explode(' ', $diagnosaUtama->icd) as $icd) <!-- Memecah string ICD menjadi array -->
                        <input type="text" name="icdD[]" class="col form-control form-control-sm" value="{{ $icd }}" />
                        @endforeach --}}
                        @php
                            $icdArray = explode(' ', $diagnosaUtama->icd); // Memecah string ICD menjadi array
                        @endphp
                        <div class="row">
                            @for ($i = 0; $i < 4; $i++)
                                <input type="text" name="icdD[]" class="col form-control form-control-sm" maxlength="1"
                                    value="{{ isset($icdArray[$i]) ? $icdArray[$i] : '' }}" />
                            @endfor
                        </div>
                    </div>
                </div>
            ...
            <div class="row mb-5">
                <div class="col-10">
                    ...
                    <textarea id="editor7" name="karmobiditas" class="form-control" style="height: 100px">{{ is_array($karmobiditasLama) ? implode(', ', $karmobiditasLama) : $karmobiditasLama }}</textarea>
                </div>

                <div class="col col-2">
                    <p>ICD 10</p>
                    @php
                        $icdArray = isset($karmobiditas->icd) ? explode(' ', $karmobiditas->icd) : [];
                    @endphp
                    @for ($i = 0; $i < 6; $i++)
                        <div class="row">
                            @for ($j = 0; $j < 4; $j++)
                                ...
                                <input type="text" name="icdK[]" class="col form-control form-control-sm"
                                    value="{{ isset($icdArray[$index]) ? $icdArray[$index] : '' }}" />
                            @endfor
                        </div>
                    @endfor
                </div>
            ...
            <div class="row mb-5">
                <div class="col-10">
                    ...
                    <textarea id="editor8" name="terapi" class="form-control" style="height: 100px">{{ is_array($terapiLama) ? implode(', ', $terapiLama) : $terapiLama }}</textarea>
                </div>
                <div class="col col-2">
                    <p>ICD 9 CM</p>
                    @php
                    $icdArray = isset($terapi->icd) ? explode(' ', $terapi->icd) : [];
                @endphp
                @for ($i = 0; $i < 3; $i++)
                    <div class="row">
                        @for ($j = 0; $j < 3; $j++)
            ...
            <div class="row mb-5">
                ...
                <div class="form-floating">
                    <textarea id="editor9" class="form-control" name="intruksi" style="height: 100px">{{ old('intruksi', $discharge->intruksi) }}</textarea>
                </div>
            </div>
