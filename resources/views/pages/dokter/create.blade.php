...
<div class="card mb-4">
  <div class="card-header d-flex align-items-center justify-content-between">
      <h5 class="mb-0">Tambah <span class="bold text-primary text-uppercase">{{ $category->name }}</span></h5>
  </div>
  <div class="card-body">
      <form method="POST" action="{{ route('dokter.store', $category->id) }}">
          @csrf
            <div class="row mb-3">
                <label class="col-sm-2 col-form-label" for="basic-default-name">Kode Dokter</label>
                <div class="col-sm-10">
                <input type="text" name="kode" class="form-control" id="basic-default-name" value="{{ old('kode') }}" required />
                </div>
            </div>
            <div class="row mb-3">
            ...
            <div class="row mb-3">
              ...
              <div class="col-sm-10">
                  <input type="text" name="telp" class="form-control" id="basic-default-name" value="{{ old('telp') }}" required />
              </div>
            </div>
            <div class="row mb-3">
              <label class="col-sm-2 col-form-label" for="basic-default-name">Alamat</label>
              <div class="col-sm-10">
                <textarea class="form-control" name="alamat" id="floatingTextarea2" style="height: 100px">{{ old('alamat') }}</textarea>
              </div>
            </div>
            <div class="row mb-3">
