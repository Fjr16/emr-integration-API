...
<div class="card mb-4">
  ...
  <div class="card-body">
      <form method="POST" action="{{ route('action/members.update', $item->id) }}">
        @method('PUT')
          @csrf
          <div class="row mb-3">
            <label class="col-sm-2 col-form-label" for="basic-default-name">Jenis Tindakan</label>
            <div class="col-sm-10">
                <select class="form-select" name="action_id" aria-label="Default select example">
                    <option selected disabled>Pilih</option>
                    @foreach ($action as $action)
                      @if (old('action_id', $item->action_id) == $action->id)
          ...
          <div class="row mb-3">
              ...
              <div class="col-sm-10">
                  <input type="text" name="code_icd" class="form-control" id="basic-default-name"  value="{{ old('code_icd', $item->code_icd) }}" required />
              </div>
          </div>
          <div class="row mb-3">
              <label class="col-sm-2 col-form-label" for="basic-default-name">Tarif Umum</label>
              <div class="col-sm-10">
                  <input type="number" name="tarif_umum" class="form-control" id="basic-default-name" value="{{ old('tarif_umum', $item->tarif_umum) }}" required />
              </div>
          </div>
          <div class="row mb-3">
              <label class="col-sm-2 col-form-label" for="basic-default-name">Tarif UC</label>
              <div class="col-sm-10">
                  <input type="number" name="tarif_uc" class="form-control" id="basic-default-name" value="{{ old('tarif_uc', $item->tarif_uc) }}" required />
              </div>
          </div>
          <div class="row mb-3">
            <label class="col-sm-2 col-form-label" for="basic-default-name">Keterangan Billing</label>
            <div class="col-sm-10">
                <select class="form-select" name="billing_caption_id" aria-label="Default select example">
                    <option selected disabled>Pilih</option>
                    @foreach ($billing as $billing)
                        @if (old('billing_caption_id', $item->billing_caption_id) == $billing->id)
          ...
          <div class="row mb-3">
            ...
            <div class="col-sm-10">
                <select class="form-select" name="tanggungan" aria-label="Default select example">
                    <option selected>Pilih</option>
                    @foreach ($tanggungan as $t)
                    @if (old('tanggungan', $item->tanggungan) == $t)
