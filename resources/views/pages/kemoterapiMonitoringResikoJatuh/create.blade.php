...
@section('content')
    ...
    <div class="card p-3 mt-5 ">
        ...
        <form action="{{ route('kemoterapi/monitoring/resiko/jatuh.store', $item->id) }}" method="POST" onsubmit="return confirm('Apakah Anda Yakin Ingin Melanjutkan ?')">
            @csrf
            <div class="card-body table-responsive">
                <table class="table table-bordered ">
                    ...
                    <tr>
                        ...
                        <td>Tanggal</td>
                        <td style="min-width: 300px"><input type="date" name="tanggal" value="{{ $today->format('Y-m-d') }}" class="form-control form-control-sm"></td>
                    </tr>
                    <tr>
                        <td colspan="2">
                            <select name="tipe" id="tipe" class="form-select form-control">
                                <option value="Awal Masuk Pasien (A)">Awal Masuk Pasien (A)</option>
                                <option value="Perubahan Kondisi Pasien (PK)">Perubahan Kondisi Pasien (PK)</option>
                                <option value="Transfer Pasien (T)">Transfer Pasien (T)</option>
                                <option value="Setelah Jatuh (SJ)">Setelah Jatuh (SJ)</option>
                            </select>
                        </td>
                    </tr>
                    <tr class="text-center">
                            ...
                            <td colspan="2">
                                <div class="d-grid cols-1 justify-content-center">
                                    ...
                                    <div class="form-check form-check-inline {{ $loop->first ? '' : 'mt-3' }}">
                                        @if ($usia <= 12)
                                            <input class="form-check-input" name="data[{{ $index }}][skor]" value="{{ $skor }}" type="radio" id="{{ $item['faktor_resiko'] }}{{ $loop->iteration }}" onclick="hitungTotalSkor()" {{ $loop->last ? 'checked' : '' }}/>
                                        @else
                                            <input class="form-check-input" name="data[{{ $index }}][skor]" value="{{ $skor }}" type="radio" id="{{ $item['faktor_resiko'] }}{{ $loop->iteration }}" onclick="hitungTotalSkor()" {{ $loop->first ? 'checked' : '' }}/>
                                        @endif
                    ...
                    <tr>
                        ...
                        <td><input type="text" name="total_skor" id="totalSkor" value="{{ $usia <= 12 ? '7' : '0' }}" class=" form-control form-control-sm" @readonly(true)></td>
                    </tr>
                    <tr>
                        <td colspan="3">
                            <h6 class="m-0">Nilai Skor (RR/RS/RT)</h6>
                        </td>
                        <td><input type="text" name="nilai_skor" id="nilaiSkor" class="form-control form-control-sm" @readonly(true)></td>
                    </tr>
                    <tr>
                        <td colspan="2">
