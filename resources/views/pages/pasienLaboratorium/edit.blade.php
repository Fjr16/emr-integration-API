...
@if (session()->has('success'))
  ...
  <div class="card">
    <div class="card-header">
      <h5 class="m-0">Edit Hasil Pemeriksaan</h5>
    </div>
    <form action="{{ route('laboratorium/patient/hasil.update', $item->id) }}" method="POST">
    @method('PUT')
    @csrf
    <div class="card-body">
      <div class="row">
        ...
        <div class="col-6">
          <div class="row mb-3">
            <label for="nolabor" class="col-form-label col-3">No. Labor</label>
            <div class="col-9">
              <input type="text" id="nolabor" class="form-control" value="{{ $item->nomor_reg_lab ?? '' }}" disabled>
            </div>
          </div>
          <div class="row mb-3">
            <label for="tanggal" class="col-form-label col-3">Tanggal Periksa</label>
            <div class="col-9">
              <input type="date" id="tanggal" name="tanggal" value="{{ $item->tanggal_periksa ?? '' }}" class="form-control">
            </div>
          </div>
          <div class="row mb-3">
        ...
        @foreach ($dataKategori as $itemCategory)
          @if (in_array($itemCategory->id, $categoryIds))   
            <div class="col-6 mb-5">
              ...
              <table class="table">
                <tbody>
                  @foreach ($item->laboratoriumPatientResultDetails as $index => $detail)
                      ...
                      @if ($detail->laboratoriumRequestMasterVariable->laboratoriumRequestCategoryMaster->id == $itemCategory->id)
                        <tr>
                          ...
                          @if ($detail->laboratoriumRequestMasterVariable->laboratoriumRequestMasterDetails->isEmpty())    
                            <td colspan="2">
                              <input type="hidden" class="form-control form-control-sm"  name="data[{{ $index }}][laboratorium_patient_result_detail_id]" value="{{ $detail->id }}">
                              <textarea class="form-control form-control-sm" name="data[{{ $index }}][hasil]" rows="1">{{ $detail->value ?? '' }}</textarea>
                            </td>
                          @else
                            <td>
                              <input type="hidden" class="form-control form-control-sm"  name="data[{{ $index }}][laboratorium_patient_result_detail_id]" value="{{ $detail->id }}">
                              <input type="text" id="hasil{{ $detail->laboratoriumRequestMasterVariable->id }}" class="form-control form-control-sm {{ $detail->kondisi_kritis ? 'border border-danger' : ''  }}" value="{{ $detail->value ?? '' }}" name="data[{{ $index }}][hasil]" onchange="checkKondisiKritis({{ $detail->laboratoriumRequestMasterVariable->id }}, '{{ $item->queue->patient->jenis_kelamin }}')">
                            </td>
                            <td>
          ...
          <table class="table table-bordered mt-0">
            <tbody>
              <tr>
                ...
                <td colspan="2">
                  <textarea name="kesan" class="form-control form-control-sm" cols="5" rows="1">{{ $item->kesan ?? '' }}</textarea>
                </td>
              </tr>
              <tr>
                <td colspan="2">Anjuran</td>
                <td colspan="2">
                  <textarea name="anjuran" class="form-control form-control-sm" cols="5" rows="1">{{ $item->anjuran ?? '' }}</textarea>
                </td>
              </tr>
            </tbody>
          </table>
          ...
          <table class="table table-bordered mt-0">
            <tbody>
              <tr>
                ...
                <td>
                  <input type="datetime-local" id="tgl-pengambilan-sampel" class="form-control form-control-sm" name="tgl_ambil_sampel" value="{{ $item->tgl_pengambilan_sampel ?? '' }}" >
                </td>
              </tr>
              <tr>
                <td>Tanggal Pemeriksaan Selesai</td>
                <td>
                  <input type="datetime-local" id="tgl-pemeriksaan-selsai" class="form-control form-control-sm" name="tgl_periksa_selesai" value="{{ $item->tgl_pemeriksaan_selesai ?? '' }}" >
                </td>
              </tr>
            </tbody>
          </table>
          ...
          <table class="table">
            <tbody id="kondisi-kritis">
              @php
                $arrNilaiKritis = [];
              @endphp
              @foreach ($item->laboratoriumPatientResultDetails->where('kondisi_kritis', true) as $detail)
              <tr id="tr{{ $detail->laboratoriumRequestMasterVariable->id }}">
                <td style="display: none;">
                  <input type="hidden" id="nilai_kritis_variabel_id{{ $detail->id }}" name="nilai_kritis_variabel_id[]" value="{{ $detail->id }}" >
                </td>
                @php
                  $arrNilaiKritis[] = $detail->laboratoriumRequestMasterVariable->name;
                @endphp
              </tr>
              @endforeach
              <tr>
                <td>Nilai Kritis</td>
                <td colspan="4">
                  <input type="text" id="nilai-kritis" class="form-control form-control-sm" value="{{ implode(', ' , $arrNilaiKritis) }}" disabled>
                </td>
              </tr>
              <tr>
                ...
                <td>Jam</td>
                <td>
                  <input type="time" id="jam" class="form-control form-control-sm" name="jam_pelaporan_kritis" value="{{ $item->jam_pelaporan_kritis, $today->format('H:i') }}" >
                </td>
                <td>Nama</td>
                <td>
