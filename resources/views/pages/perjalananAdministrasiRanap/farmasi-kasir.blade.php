                ...
                <table class="table">
                    <thead>
                        <tr class="text-nowrap bg-dark">
                            <th><input type="hidden" value="{{ $data2->category }}" name="farmasiKasir">{{ $data2->category }}
                            </th>
                            <th></th>
                            <td></td>
                        </tr>
                    ...
                    <tbody>
                        @foreach ($data3 as $item)
                            <tr>
                                <td>{{ $item->name }}</td>
                                @if ($item->name == 'JAM PENYERAHAN STATUS' || $item->name == 'JAM VISITE DOKTER')
                                    <td><input type="time" value="{{ $item->value }}"
                                            name="rekamMedis[{{ $item->name }}]" class="form-control" required>
                                    </td>
                                @else
                                    <td><input type="radio" value="ada" name="rekamMedis[{{ $item->name }}]"
                                            {{ $item->value == 'ada' ? 'checked' : '' }}> Ada
                                    </td>
